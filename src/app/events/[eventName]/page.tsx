...
export default function SingleEventPage() {
  ...
  // Use our real-time cloud hook!
  const { items: dbItems, loading } = useEventItems(rawName);
  ...
  const getItemsForCategory = (catId: string) => dbItems.filter(item => item.category === catId);

  // --- NEW CLOUD FUNCTIONS ---
  const handleAddEntry = async () => {
    if (!activeCategory || !newItem.trim()) return;
    
    // Instantly inserts a new row into the cloud database
  ...
  const moveTask = async (id: string, newCategory: string) => {
  ...
  const pendingCount = getItemsForCategory('tasks').length;
  const doneCount = getItemsForCategory('taskdone').length;
  const totalTasks = pendingCount + doneCount;
  const progressPercentage = totalTasks === 0 ? 0 : Math.round((doneCount / totalTasks) * 100);
        ...
        {CATEGORIES.map(cat => {
          ...
          return (
            <button 
              key={cat.id} 
              onClick={() => {
                if (cat.id === 'outfit') router.push(`/events/${rawName}/outfits`);
                else if (isExpenses) router.push('/budget');
                else {
                  setActiveCategory(cat.id);
                  setIsDialogOpen(true);
                }
            ...
            >
              ...
              <div className="text-sm text-slate-500 flex-1 space-y-1 mt-2">
                {items.length === 0 && !isExpenses ? (
                  <span className="italic text-slate-400">Empty</span>
                ) : (
                  items.slice(0, 2).map((item) => (
                    <div key={item.id} className="truncate">
                      {item.content.startsWith('ImageLink:') ? '📷 Attached Image' : `• ${item.content}`}
                    </div>
                  ))
                )}
                {items.length > 2 && <div className="text-xs font-bold text-slate-400">+{items.length - 2} more</div>}
          ...
          <div className="mt-4 max-h-[400px] overflow-y-auto space-y-2">
            {getItemsForCategory(activeCategory || '').map((item) => (
              <div key={item.id} className="flex justify-between items-center p-3 border rounded bg-slate-50">
                <div className="flex-1 overflow-hidden pr-4">
                  {item.content.startsWith('ImageLink:') ? renderThumbnail(item.content) : <span className="text-sm">{item.content}</span>}
                </div>
                <div className="flex gap-2">
                  {activeCategory === 'tasks' && <button onClick={() => moveTask(item.id, 'taskdone')} className="text-green-600 p-1 hover:bg-green-100 rounded"><Check className="w-4 h-4"/></button>}
                  {activeCategory === 'taskdone' && <button onClick={() => moveTask(item.id, 'tasks')} className="text-amber-600 p-1 hover:bg-amber-100 rounded"><RotateCcw className="w-4 h-4"/></button>}
                  <button onClick={() => deleteItem(item.id)} className="text-red-500 p-1 hover:bg-red-100 rounded"><Trash2 className="w-4 h-4"/></button>
                </div>
              </div>
            ))}
          </div>
