          ...
          <div className="p-4 space-y-4 min-h-[300px]">
            {ongoingTasks.map((task: any) => {
              ...
              return (
                <div 
                  key={task.id} 
                  className={`bg-white border p-4 rounded-xl shadow-sm flex flex-col sm:flex-row gap-4 sm:justify-between sm:items-start group transition-colors ${overdue ? 'border-red-300 bg-red-50/30' : 'border-slate-200'}`}
                >
                  ...
                  {editingTaskId === task.id ? (
                    ...
                    <div className="flex-1">
                      ...
                      {task.due_date && (
                        <p className={`text-xs flex items-center gap-1 mt-1.5 font-medium ${overdue ? 'text-red-600' : 'text-slate-400'}`}>
                          <Calendar className={`w-3.5 h-3.5 ${overdue ? 'text-red-500' : 'text-emerald-500'}`} /> 
                          {overdue ? "Overdue:" : "Due by"} {formatDate(task.due_date)}
                        </p>
                      )}
