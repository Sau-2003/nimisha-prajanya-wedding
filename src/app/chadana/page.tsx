...
function ChadanaCard({ 
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const currentImages = gift.image_urls || [];
    onUpdate(gift.id, { image_urls: [...currentImages, ...newBase64Images] });
  };

  const removeImage = (indexToRemove: number) => {
    const currentImages = gift.image_urls || [];
    const updatedImages = currentImages.filter((_: string, idx: number) => idx !== indexToRemove);
    onUpdate(gift.id, { image_urls: updatedImages });
  };
      ...
      <div className="w-full px-4 md:px-10 pb-6">
        ...
        {gift.image_urls && gift.image_urls.length > 0 && (
          <div className="mt-4 flex flex-wrap gap-4 px-4">
            {gift.image_urls.map((url: string, idx: number) => (
              <div key={idx} className="relative inline-block group/image">
                <img 
                  src={url} 
                  alt={`Attachment ${idx + 1}`} 
                  className="h-32 w-auto rounded-lg border border-slate-200 object-cover cursor-pointer hover:opacity-90 shadow-sm transition-opacity"
    ...
    <div className="p-6 md:p-12 max-w-4xl mx-auto min-h-screen space-y-8">
      ...
      <div className="space-y-6">
        {localChadana.length === 0 ? (
           <div className="text-center py-12 text-slate-400 italic">No chadana tracked yet. Click "Add Chadana" to get started!</div>
        ) : (
