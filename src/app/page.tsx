...
export default function Dashboard() {
  ...
  useEffect(() => {
    const checkUser = async () => {
      // 1. Immediately check URL hash or search params for password recovery token
      const urlCheck = window.location.hash || window.location.search;
      if (urlCheck.includes("type=recovery") || urlCheck.includes("access_token")) {
        // Verify if it's actually a recovery session from Supabase
        const { data: { session } } = await supabase.auth.getSession();
        if (session || urlCheck.includes("type=recovery")) {
          setAuthMode("update-password");
          setIsChecking(false);
          return;
        }
      ...
      // 2. Listen for Supabase auth state change events
      const { data: { subscription } } = supabase.auth.onAuthStateChange((event) => {
        if (event === "PASSWORD_RECOVERY") {
          setAuthMode("update-password");
          setIsChecking(false);
