...
//         // Check if we have an active recovery session or flag
...
//       // If the URL has the recovery flag or a hash token from Supabase verification
//       if (
//         queryParams.get("type") === "recovery" || 
...
export default function Dashboard() {
  // --- THE TRAP ---
  // We capture the exact URL the millisecond the page opens, BEFORE Supabase erases it!
  const [initialUrl] = useState(() => typeof window !== "undefined" ? window.location.href : "");
  ...
  useEffect(() => {
    ...
    const checkUser = async () => {
      // 1. Check our perfectly captured URL snapshot
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      if (event === "PASSWORD_RECOVERY") {
        ...
        // Do not interrupt them if they are trying to update their password
        setAuthMode((currentMode) => {
          if (currentMode !== "update-password" && !initialUrl.includes("type=recovery")) {
            setIsAuthorized(true);
            fetchEvents();
          }
          return currentMode;
