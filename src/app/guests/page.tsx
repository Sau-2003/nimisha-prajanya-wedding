...
//           <input className="border p-2 rounded" placeholder="Family Name" value={newGuest.family} onChange={e => setNewGuest({...newGuest, family: e.target.value})} />
//           <input className="border p-2 rounded" placeholder="No. of People" type="number" value={newGuest.count} onChange={e => setNewGuest({...newGuest, count: e.target.value})} />
...
//             </>
//           )}
//           <Button onClick={handleAdd} className="bg-emerald-600"><Plus className="w-4 h-4 mr-2" /> Add Guest</Button>
//         </div>
...
//                     <td className="p-3">{g.arrival_date? new Date(g.arrival_date).toLocaleDateString("en-GB"): "-"}</td>
//                   </>
//                 )}
//                 <td className="p-3">
...
export default function GuestsPage() {
  ...
  // 1. Updated newGuest state with mobile and arrivalTime
  ...
  // 3. Updated createEntry to save mobile_no and arrival_time
  const handleAdd = async () => {
    ...
    const createEntry = (category: string) => ({
      ...
      count: newGuest.count,
      room_no: category === activeTab ? newGuest.room : "",
      // Force empty strings to be null to prevent Supabase type errors
    ...
    // 3. Clear form and fetch data if successful
    ...
    <div className="p-8 max-w-6xl mx-auto">
      ...
      <div className="border rounded-lg p-6 bg-white shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6 bg-slate-50 p-4 rounded-lg">
          ...
          {/* 2. Added new inputs after Room No (arranged logically in the grid) */}
          {activeTab.includes("Staying") && (
            <>
              <input className="border p-2 rounded" placeholder="Room No" value={newGuest.room} onChange={e => setNewGuest({...newGuest, room: e.target.value})} />
              <input className="border p-2 rounded" placeholder="Mobile Number" value={newGuest.mobile} onChange={e => setNewGuest({...newGuest, mobile: e.target.value})} />
              <input type="time" className="border p-2 rounded" value={newGuest.arrivalTime} onChange={e => setNewGuest({...newGuest, arrivalTime: e.target.value})} />
              <input className="border p-2 rounded" placeholder="Hotel" value={newGuest.hotel} onChange={e => setNewGuest({...newGuest, hotel: e.target.value})} />
              <input className="border p-2 rounded" placeholder="Origin" value={newGuest.origin} onChange={e => setNewGuest({...newGuest, origin: e.target.value})} />
              <input type="date" className="border p-2 rounded" value={newGuest.arrival} onChange={e => setNewGuest({...newGuest, arrival: e.target.value})} />
            </>
          ...
          {/* 5. Added the copy options checkboxes */}
          {activeTab.includes("Staying") && (
        ...
        {/* 4. Updated Table Headers & Body strictly to requested layout */}
                ...
                <tr key={g.id} className="border-b hover:bg-slate-50">
                  {editingId === g.id ? (
                    ...
                    <>
                      ...
                      {activeTab.includes("Staying") && (
                        <>
                          <td className="p-2"><input className="border w-full p-1" value={editForm.room_no} onChange={e => setEditForm({...editForm, room_no: e.target.value})} /></td>
                          <td className="p-2"><input className="border w-full p-1" value={editForm.mobile_no} onChange={e => setEditForm({...editForm, mobile_no: e.target.value})} /></td>
                          <td className="p-2"><input type="time" className="border w-full p-1" value={editForm.arrival_time} onChange={e => setEditForm({...editForm, arrival_time: e.target.value})} /></td>
                        </>
                      )}
                      <td className="p-2"><input className="border w-full p-1" value={editForm.family} onChange={e => setEditForm({...editForm, family: e.target.value})} /></td>
                      <td className="p-2"><input className="border w-full p-1" type="number" value={editForm.count} onChange={e => setEditForm({...editForm, count: e.target.value})} /></td>
                      {activeTab.includes("Staying") && (
                        <>
                          <td className="p-2"><input type="date" className="border w-full p-1" value={editForm.arrival_date} onChange={e => setEditForm({...editForm, arrival_date: e.target.value})} /></td>
                          <td className="p-2"><input className="border w-full p-1" value={editForm.origin_place} onChange={e => setEditForm({...editForm, origin_place: e.target.value})} /></td>
                          <td className="p-2"><input className="border w-full p-1" value={editForm.hotel_name} onChange={e => setEditForm({...editForm, hotel_name: e.target.value})} /></td>
                        </>
                      )}
