...
export default function AllOutfitsPage() {
  ...
  const [activeTab, setActiveTab] = useState<string>("Main Look");

  // Fetch ALL outfits across ALL events from event_items table matching event page logic
  useEffect(() => {
    const fetchAllOutfits = async () => {
      ...
      const { data, error } = await supabase
        .from('event_items')
        .select('*')
        .like('category', 'outfit_%'); // Matches event outfit category structure
      ...
      if (error) {
        ...
        const mappedData: ParsedOutfit[] = data.map((item: any) => ({
          ...
          category: item.category.replace('outfit_', ''), 
          caption: item.text || "Outfit Image",
        ...
        if (uniqueTabs.length > 0 && !uniqueTabs.includes("Main Look")) {
    ...
    <div className="min-h-screen p-6 md:p-12 max-w-7xl mx-auto space-y-8">
      ...
      <div>
        ...
        <p className="text-slate-500 mt-2">
          View and manage all planned outfits grouped by event.
        </p>
      </div>
      ...
      {/* Main Content */}
      {loading ? (
        <div className="py-20 text-center text-emerald-600 font-medium animate-pulse">Loading outfits...</div>
      ) : sortedEvents.length === 0 ? (
        <div className="py-20 text-center border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50">
          <p className="text-slate-400 italic">No outfits have been added to any event yet.</p>
        </div>
      ) : !hasOutfitsInActiveTab ? (
         <div className="py-20 text-center border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50">
           <p className="text-slate-400 italic">No outfits found for "{activeTab}" across any events.</p>
         </div>
      ) : (
      ...
      {/* --- FULL SCREEN PHOTO LIGHTBOX (Matches Event Outfit Page) --- */}
