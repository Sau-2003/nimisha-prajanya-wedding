...
import { Check, ClipboardList, Trash2, RotateCcw, Plus, Pencil, X, Calendar, Image as ImageIcon, User, Settings, ChevronDown, Bold, Italic, Strikethrough } from "lucide-react";
...
export default function TasksPage() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<{ type: 'task', id: string } | { type: 'teamMember', name: string } | null>(null);
  ...
  const handleAddTask = async () => {
    const plainTextContent = newTaskText.replace(/<[^>]*>?/gm, '').trim(); // Prevent saving empty HTML
  ...
  const handleConfirmDelete = async () => {
    ...
    if (deleteTarget.type === 'task') {
      ...
      fetchData();
    } else if (deleteTarget.type === 'teamMember') {
      await deleteTeamMember(deleteTarget.name);
    }
  ...
  const linkifyHtml = (htmlText: string) => {
    ...
    return htmlText.replace(urlRegex, '<a href="$1" target="_blank" rel="noopener noreferrer" class="text-emerald-600 hover:underline break-all">$1</a>');
  };
                ...
                <div key={task.id} className={`bg-white border p-4 rounded-xl shadow-sm flex flex-col sm:flex-row gap-4 sm:justify-between sm:items-start group transition-colors ${overdue ? 'border-red-300 bg-red-50/30' : 'border-slate-200'}`}>
                  {editingTaskId === task.id ? (
                    <div className="flex-1 flex flex-col gap-2 w-full">
                      {/* Rich Text Editor for inline editing */}
                      <EditableCell
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 items-center">
                        <input 
                          ...
                          className="text-sm border rounded px-2 h-9 text-slate-600 outline-none focus:border-emerald-500 w-full bg-white"
                        />
                    ...
                    <div className="flex-1">
                      ...
                      <div 
                        className="text-slate-700 whitespace-pre-wrap break-words [&_b]:font-bold [&_strong]:font-bold [&_i]:italic [&_em]:italic [&_strike]:line-through [&_s]:line-through"
                        dangerouslySetInnerHTML={{ __html: linkifyHtml(task.text) }}
                        onClick={(e) => {
                  ...
                  {editingTaskId !== task.id && (
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => setDeleteTarget({ type: 'task', id: task.id })} className="p-2 bg-red-50 text-red-500 hover:bg-red-100 rounded-lg"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  )}
                </div>
              );
            ...
            {completedTasks.map((task: any) => (
              <div key={task.id} className="bg-white border p-4 rounded-xl shadow-sm flex justify-between items-start group">
                <div className="flex-1">
                  ...
                  <div 
                    className="text-slate-400 line-through whitespace-pre-wrap break-words [&_b]:font-bold [&_strong]:font-bold [&_i]:italic [&_em]:italic [&_strike]:line-through [&_s]:line-through"
                    dangerouslySetInnerHTML={{ __html: linkifyHtml(task.text) }}
                    onClick={(e) => {
                      if ((e.target as HTMLElement).tagName.toLowerCase() === 'a') {
                        e.stopPropagation();
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4 pt-2">
            {/* Rich Text Editor for adding task */}
            <EditableCell
              value={newTaskText}
              onChange={(val) => setNewTaskText(val)}
              placeholder="What needs to be done?"
              className="w-full border p-3 rounded-lg outline-none focus:ring-2 focus:ring-emerald-500 min-h-[80px] bg-white text-slate-700"
              autoFocus
            />
                  ...
                  {editingMemberIndex === index ? (
                    ...
                    <>
                      ...
                      <div className="flex gap-1">
                        ...
                        <button onClick={() => setDeleteTarget({ type: 'teamMember', name: member })} className="p-1.5 text-red-400 hover:text-red-600"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
                    </>
                  )}
                </div>
